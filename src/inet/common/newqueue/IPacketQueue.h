...
class INET_API IPacketQueue
{
  ...
  public:
    ...
    /**
     * Pushes a packet into the queue.
     */
    virtual void pushPacket(Packet *packet) = 0;

    /**
     * Pops a packet from the queue. The queue must not be empty.
     */
    virtual Packet *popPacket() = 0;

    /**
     * Removes a packet from the queue. The queue must contain the packet.
     */
